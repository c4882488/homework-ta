...
for i in range(len(files)):
    ...
    if files[i].split("_")[0] in double_check:
        double_check_score = 1
    ...
    data.append(
        [
            ...
            double_check_score,  # 雙重檢查
            double_check_magnification,  # 雙重檢查倍率
            ...
            len(import_data) - ((nessary_data["length"]+nessary_programs_data['length'])*1.5),  # 使用的import數量
            code_lines  # 代碼行數
        ] + comments
    )
...
df = pd.DataFrame(data, columns=["作業檔案名稱", "與誰相似", "相似次數", "平均相似度", "相似基數", "相似倍率", "是否為簡字基數", "簡字倍率", "雙重檢查", "雙重檢查倍率", "註解總行數", "註解總字數", "平均註解字數", "使用的import", "未使用的import", "使用的import數量","代碼行數"]+comments)
...
df2 = df[["作業檔案名稱", "分數"]]
df2["作業檔案名稱"] = df2["作業檔案名稱"].str.split("_", expand=True)[0]
